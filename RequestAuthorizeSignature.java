...
public class RequestAuthorizeSignature implements HttpRequestFilter, RequestSigner {
   ...
   public HttpRequest filter(HttpRequest request) throws HttpException {
      ...
      if (host.indexOf("aliyuncs") != -1) {
          request = replaceHostHeader(request);
      }
   ...
   HttpRequest replaceHostHeader(HttpRequest request) {
      String host = request.getEndpoint().getHost();
      String eprawpath = request.getEndpoint().getRawPath();
//      signatureLog.info("czq replaceHostHeader  eprawpath %s\ntoString:%s\n", eprawpath, request.getEndpoint().toASCIIString());
      if (host == null || host.length() == 0 || eprawpath == null || eprawpath.length() <= 1) {
          return request;
      }
      //String strquery = request.getEndpoint().getQuery();
      //String strfragment = request.getEndpoint().getRawFragment();

      int sindex = eprawpath.indexOf("/", 1);
      String bucketname = "";
      String newpath = "";
      if (sindex != -1) {
          bucketname = eprawpath.substring(1, sindex);
          newpath = eprawpath.substring(sindex, eprawpath.length());
      }
      else {
          bucketname = eprawpath.substring(1, eprawpath.length());
          newpath = "/";
      }
      String newHost = bucketname + "." + host;
//      signatureLog.info("czq newhost: %s\n, host:%s\n", newHost,  host);
      URI redirect = uriBuilder(request.getEndpoint()).host(newHost).path(newpath).build();

      request = request.toBuilder().endpoint(redirect).build();
   ...
   public String createStringToSign(HttpRequest request) {
     ...
     // signatureLog.info("czq createStringToSign: %s\n", buffer.toString());
   ...
   void appendUriPath(HttpRequest request, StringBuilder toSign) {
      String host = request.getEndpoint().getHost();
//      signatureLog.info("czq appendUriPath host: %s\n", host);
      if (host.indexOf("aliyuncs") != -1 && !host.equals("oss-cn-shanghai.aliyuncs.com")) {
          int sindex = host.indexOf(".");
          String bucket = "";
          if (sindex != -1) {
              bucket = host.substring(0, sindex);
              toSign.append("/").append(bucket);
          }
      }
